...
exports.getTopProjects = function(username,handler) {
	...
	return db.instance("default").find("SELECT COUNT(Task) AS top,Task FROM daylog WHERE "+sql+"Date > datetime('now', '-1 month') GROUP BY Task ORDER BY top DESC LIMIT 10",args,function(err,projectList){
		if ( err ) {
