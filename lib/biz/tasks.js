...
//	db.instance("default").find("daylog",{Date:binDate},{},{},function(err,rows){
	db.instance("default").find("SELECT * FROM daylog WHERE Date=?",[binDate],function(err,rows){
		if ( err ) {
...
exports.getTopProjects = function(username,handler) {

	var
		sql		= username ? " WHERE Username=?" : "",
		args	= username ? [username] : null;

	return db.instance("default").find("SELECT COUNT(*) AS top,Task FROM daylog "+sql+" ORDER BY top LIMIT 10",args,function(err,projectList){
		if ( err ) {
...
exports.getCurrentTask = function(user,handler) {
	...
	// Get user daylog
	return this.list({Username:user,Date:getToday()},[["Order",1]],function(err,tasks){
		if ( err ) {
...
// Open a task
exports.openTask = function(user,task,handler) {
	...
	log.info("Opening task '"+(typeof task == "string"?task:task.Task)+"' at "+now+"...");
	...
	return async.ifThen(typeof task == "string",
		...
		function(next) {
			...
			// Locate it
//			return db.instance("default").find("daylog",{Username: user, Date: today, Task: task},{},{sort:[['Order',-1]]},function(err,ftasks){
			return db.instance("default").find("SELECT * FROM daylog WHERE Username=? AND Date=? AND Task=? ORDER BY OrderNumber DESC",[user,today,task],{},function(err,ftasks){
				if ( err ) {
...
exports.closeTask = function(user,task,handler) {
	...
	return async.ifThen(typeof task == "string",
		...
		function(next) {
			...
			// Locate it
//			return db.instance("default").find("daylog",{Username: user, Date: today, Task: taskName},{},{sort:[['Order',-1]]},function(err,ftasks){
			return db.instance("default").find("SELECT * FROM daylog WHERE Username=? AND Date=? AND Task=? ORDER BY OrderNumber DESC",[user,today,taskName],function(err,ftasks){
				if ( err ) {
					log.error("Error trying to locate an eventually existent task named '"+task+"': ",err);
					return handler(err,null);
				}
		...
		function() {
			...
			// Edit task notes and save it
				...
				// Save it
//				return db.instance("default").update("daylog",{_id:task._id},task,function(err,ok){
				return db.instance("default").operation("UPDATE daylog SET Notes=? AND Time=? WHERE _id=?",[task.Notes,task.Time,task._id],function(err,ok){
					if ( err ) {
						log.error("Error updating task '"+task._id+"' for opening it: ",err);
						return handler(err,null);
					}
...
exports.getUserDaylog = function(user,date,handler) {
	...
	if ( typeof binDate == "string" ) {
		binDate = new Date(Date.parse(binDate+" 00:00:00"));
	}
	else {
		zeroHours(binDate);
	}

	return this.list({Username:user,Date:binDate},[["Notes",1]],function(err,tasks){
		if ( err ) {
			log.error("Error getting tasks list: ",err);
			return handler(err,null);
		}
...
//	return db.instance("default").remove("daylog",{Username:user,Date:date},function(err,ok){
	return db.instance("default").operation("DELETE FROM daylog WHERE Username=? AND Date=?",[user,date],function(err,ok){
		if ( err ) {
...
function formatTaskNotes(data) {
	...
	if ( data.times && data.times.length > 0 ) {
		data.times.forEach(function(period){
			str += "(" + (period.s?(period.s+" "):" ") + "às" + (period.e?(" "+period.e):" ") + ")\n";
		});
	}
	if ( data.text )
		str += data.text;
...
function getToday() {
	...
	return new Date(Date.parse(strDate+" 00:00:00"));
