...
// Basic auth handler
spritz.auth(/.*/,{check: function(u,p,cb){
...
spritz.on('/',function(req,res){
	...
	if ( !strDate || !strDate.match(/^\d{4}\-\d{1,2}\-\d{1,2}/) )
		strDate = (parseInt(today.getYear()+1900)+"-"+parseInt(today.getMonth()+1)+"-"+today.getDate());
	binDate = new Date(Date.parse(strDate+" 00:00:00"));
	if ( binDate > today )
...
spritz.on(/^\/reports\/custeio\/$/,function(req,res){
	...
	if ( dateFrom && dateFrom.match(/^\d{4}\-\d{1,2}\-\d{1,2}$/) ) {
		try {
			dateFrom = new Date(Date.parse(dateFrom.substr(0,7)+"/01"));
		}
		catch(ex){}
	}
	else
	...
	return bizTasks.list({Username:viewUser,Date:{$gte:dateFrom,$lt:dateTo}},[['Date',1],['Task',1]],function(err,rows){
		...
		Object.keys(projectsByDay).sort().forEach(function(day){
			...
			Object.keys(projectsByDay[day]).sort().forEach(function(project){
				...
				if ( _excludeProject(project) )
					excludedTime += projectsByDay[day][project];
			});
			// Add the projects
			Object.keys(projectsByDay[day]).sort().forEach(function(project){
				var
					pct = !_excludeProject(project) ?
		...
		return spritz.template(req,res,'reports_custeio',{
			user:       req.authUser,
			viewUser:   viewUser,
			dateFrom:   dateFrom,
			dateTo:		dateTo,
...
function _excludeProject(project) {

//	return project.match(/Re[uú]ni[aã]o (di[aá]ria|geral|com )|Backoffice( de)? log diário|custeio|gest[aã]o .*equipa|assuntos pessoais|spritz|visita |mudan[cç]a|mails? |\bequipa\b|raspb|configurar |password/i);
	return !(project||"").replace(/:.*$/,"").match(/^\s?(SAPO Desporto|SAPO Not[íi]cias(?: \w+)?|Plat Sites Especiais|Homepage|Banca de jornais(?: .*)?|SAPO Banca.*|(?:.* )?Ardina|Bricolage|SAPO Oi PP|Meta\/Cache|SAPO Flipboard|Apps Banca(?: .*)?|(?:.* )?Forma[çc][aã]o(?: .*)?)\s*$|^Plat Sites | cinema |[óo]scares|rssworks|carnaval|^feeds |Parceiros |^Site especial |SAPO Not[íi]cias |^Screenshooter/i);
